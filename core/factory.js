...
function parseCSV(text) {
  ...
  return lines.slice(1).map(line => {
    ...
    // Corrección para CSV exportado con cada fila completa encerrada entre comillas.
    // Ejemplo problemático:
    // "PN01,PICKING_PRIMER_NIVEL,""Pregunta"",""Opción A"",..."
    // El primer split devuelve 1 solo campo; aquí lo volvemos a partir internamente.
...
function cleanQuestionRow(row) {
  ...
  // Quita comillas dobles residuales si el CSV vino sobrecomillado.
  Object.keys(question).forEach(key => {
    if (typeof question[key] === "string") {
      question[key] = question[key].replace(/^"+|"+$/g, "").trim();
    }
  });
  ...
  question.correct = question.correct.replace(/^"+|"+$/g, "").trim().toUpperCase();
